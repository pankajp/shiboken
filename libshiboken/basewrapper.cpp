                              ...
                              SbkObjectType* instanceType)
{
    ...
    if (reinterpret_cast<PyTypeObject*>(Py_TYPE(wrapper)) == reinterpret_cast<PyTypeObject*>(instanceType)) {
        return wrapper;
    }
    ...
    for (ChildrenList::iterator iChild = children.begin(); iChild != children.end(); iChild++) {
        if (!((*iChild)->d && (*iChild)->d->cptr)) {
            continue;
        }
        if ((*iChild)->d->cptr[0] == wrapper->d->cptr[0]) {
            if ((void*)(Py_TYPE(*iChild)) == (void*)(instanceType)) {
